...
- New rule `leaky-top-level-use`, on by default: a `use` or `use-all`
  written at a file's top level is now reported. Carp scopes a `use` to
  ...
  idiomatic. Reported only, never fixed.
